...
public class Evaluation {
    ...
    public String toString() {
        ...
        for (Map.Entry<Variable<Integer>, Integer> entry : int_values.entrySet()) {
            if (entry.getKey().toString().equals("(int)number_of_actions") ||
                    entry.getKey().toString().equals("(int)dice_outcome") ||
                    entry.getKey().toString().equals("(int)player_pos") ||
                    false) {
                res += last;
                res += entry.getKey().toString().substring(5) + "=" + entry.getValue();
                last = ", ";
            }
        }
        last = "";
